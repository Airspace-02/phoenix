...
GITHUB_PHOENIX_BASE ?= github-phoenix
...
.PHONY: test-lint
test-lint: clean-py
	pip install flake8; python -m flake8 phoenix --filename='*.py' --max-line-length 120 --exclude __init__.py
.PHONY: test-unit
test-unit: test-requirements
	$(PYTHON) -m pytest -v --durations=20 --cov-config=.coveragerc --cov=phoenix -p no:logging
...
ifneq ($(GITLAB_CI),)
	...
	docker run --rm -t \
		-v `pwd`:/app \
		-w /app \
		$(PREFIX)/$(GITHUB_PHOENIX_BASE):latest \
